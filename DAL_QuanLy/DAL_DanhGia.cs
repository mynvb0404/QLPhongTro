...
namespace DAL_QuanLy
{
    ...
    {
        public readonly string connectionString = DBConnect.Instance.ConnectionString;
        ...
        public bool Them(DTO_DanhGia dto)
        {
            string sql = @"INSERT INTO DANHGIA (MANGUOIDG, MANGUOIDUOCDG, MAHOPDONG, DIEM, NOIDUNG, THOIGIAN)
                           VALUES (@mndg, @mnddg, @mhd, @diem, @nd, GETDATE())"; // Dùng GETDATE() của SQL cho đồng bộ
        ...
        // Lấy tất cả đánh giá về một người cụ thể (để hiển thị uy tín khi tìm người ở ghép - Chức năng 6.2 & 6.6)
        public DataTable GetDanhGiaCuaNguoiDung(int maNguoiDuocDG)
        {
            string sql = "SELECT * FROM DANHGIA WHERE MANGUOIDUOCDG = @ma"; //
            return ExecuteQuery(sql, new SqlParameter("@ma", maNguoiDuocDG));
        }
        ...
        public bool KiemTraDaDanhGia(int maHopDong)
        {
            ...
            DataTable dt = ExecuteQuery(sql, new SqlParameter("@maHD", maHopDong));
