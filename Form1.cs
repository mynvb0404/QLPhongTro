...
namespace Khách_Hàng
{
    public partial class frmKhachHang : Form
    {
        ...
        // Sự kiện click để đổ dữ liệu lên TextBox
        private void dgvKhachHang_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                txtMaKH.ReadOnly = true; // Khóa mã khi đang chọn
            }
        }

        // Nút Thêm (Mở form nhập tài khoản phụ nếu cần, ở đây giả định dùng biến tạm)
    ...
    string ho = txtHoKH.Text.Trim();
            ...
            try
            {
                ...
                if (ketQua == "THÀNH CÔNG")
                {
                    ...
                    // Xóa sạch các ô nhập liệu sau khi thêm xong
                    txtMaKH.Clear();
                    txtHoKH.Clear();
                    txtTenKH.Clear();
                    txtCCCD.Clear();
                    txtSDT.Clear();
                    txtMaKH.ReadOnly = false; // Mở khóa mã để chuẩn bị cho lần nhập mới
                }
                else
                {
